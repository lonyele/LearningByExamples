...
class Greeter(message: String, printerActor: ActorRef) extends Actor {
  ...
  def receive = {
    case WhoToGreet(who) => greeting = s"$message, 헐 $who ??????"
    case Greet => printerActor ! Greeting(greeting)
    case whatThe => printerActor ! whatTheing(greeting)
...
object Printer {
  ...
  final case class whatTheing(holly: String)
}
//#printer-messages
//#printer-companion
...
class Printer extends Actor with ActorLogging {
  ...
  def receive = {
    ...
    case whatTheing(holly) => log.info(s"whatThe...fuck???? $holly")
  }
}
//#printer-actor
...
object AkkaQuickstart extends App {
  ...
  goodDayGreeter ! WhoToGreet("wthe the fuck???")
  goodDayGreeter ! Greet
  goodDayGreeter ! whatThe
